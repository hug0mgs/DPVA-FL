...
class HARCNN(nn.Module ):
    def __init__(self, in_channels=9, dim_hidden=64*26, num_classes=6, conv_kernel_size=(1, 9), pool_kernel_size=(1, 2)):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.ReLU(inplace=False), # CORRIGIDO
...
class Digit5CNN(nn.Module ):
    def __init__(self):
        ...
        self.encoder.add_module("relu1", nn.ReLU(inplace=False)) # CORRIGIDO
        ...
        self.encoder.add_module("relu2", nn.ReLU(inplace=False)) # CORRIGIDO
        ...
        self.encoder.add_module("relu3", nn.ReLU(inplace=False)) # CORRIGIDO

        self.linear = nn.Sequential()
        self.linear.add_module("fc1", nn.Linear(8192, 3072))
        self.linear.add_module("bn4", nn.BatchNorm1d(3072))
        self.linear.add_module("relu4", nn.ReLU(inplace=False)) # CORRIGIDO
        ...
        self.linear.add_module("relu5", nn.ReLU(inplace=False)) # CORRIGIDO
    ...
    def __init__(self, in_features=1, num_classes=10, dim=1024):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_features,
                        ...
                        stride=1,
                        bias=True),
            nn.ReLU(inplace=False), # CORRIGIDO: inplace=False
...
class FedAvgMLP(nn.Module):
    def __init__(self, in_features=784, num_classes=10, hidden_dim=200):
        ...
        self.act = nn.ReLU(inplace=False) # CORRIGIDO
...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(x) # F.relu não tem inplace=True por padrão
...
class LSTMNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.relu(out[:,-1,:]) # CORRIGIDO
