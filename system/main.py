...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        # select algorithm
        algo = args.algorithm.lower() # Converte para minúsculas para evitar erros
        ...
        # CORREÇÃO: Adicionado FedAdg_DP para corresponder ao seu comando
        elif algo == "fedadg_dp":
            if args.dp_mode == 'none':
            ...
            if args.dp_mode == 'none':
                ...
                # Permite que FedALA também seja executado com DP
                server = FedALA_DP(args, i)
    ...
    # A chamada para average_data foi removida para evitar avisos de arquivos não encontrados.
    # O cálculo de média e desvio padrão deve ser feito separadamente após a conclusão de todos os experimentos.
...
if __name__ == "__main__":
    ...
    # ==================== NOVOS ARGUMENTOS PARA O SEU PROJETO DE TESE ====================
    ...
    parser.add_argument('-dpe', '--dp_epsilon', type=float, default=50.0,
                        help='Orçamento de privacidade Epsilon (usado no modo fixed e como base no adaptive).')
    parser.add_argument('-dpd', '--dp_delta', type=float, default=1e-5,
                        help='Delta para (epsilon, delta)-DP. Geralmente 1/N, onde N é o tamanho do dataset.')
    ...
    parser.add_argument('-dpgn', '--dp_max_grad_norm', type=float, default=10.0,
                        help='Norma máxima para o clipping de gradientes por amostra (parâmetro do Opacus).')
    
    # Parâmetros para o modo adaptativo
    parser.add_argument('-dpemax', '--dp_epsilon_max', type=float, default=50.0,
                        help='Epsilon máximo para o modo adaptativo.')
    parser.add_argument('-dpemin', '--dp_epsilon_min', type=float, default=5.0,
                        help='Epsilon mínimo para o modo adaptativo.')
    
    parser.add_argument('-sm', '--secure_mode', action='store_true', help='Ativar modo seguro do Opacus (mais lento, mas mais seguro).')
